...
--model /work/models/Qwen/Qwen3-30B-A3B-GPTQ-Int4_Repacked \
--dataset-name random \
--dataset-path  /work/models/ShareGPT_V3_unfiltered_cleaned_split.json \
--seed 42 \
--host 0.0.0.0 \
--port 10020 \
...
def get_dataset(args, tokenizer, model_id=None):
    ...
    if args.dataset_name == "sharegpt":
        ...
        input_requests = sample_random_requests(
            ...
            repeat_mode=getattr(args, 'repeat_mode', 'duplicate'),
        )
...
def repeat_sample_requests(requests: List[DatasetRow], repeat_count: int, mode: str) -> List[DatasetRow]:  
    """  
    Repeat each DatasetRow in the list for a specified number of times.  
      
    Args:  
        requests: A list of DatasetRow objects to be repeated.  
        repeat_count: The number of times each request is repeated.  
        mode: The mode of repetition ('duplicate', 'sequential', 'interleaved').  
      
    Returns:  
        A list of repeated DatasetRow objects.  
    """  
    if mode == "duplicate":  
        # 每个 prompt 重复 N 次后再处理下一个  
        repeated_requests = []  
        ...
        raise ValueError(f"Invalid mode: {mode}, only support 'duplicate', 'sequential', 'interleaved'")
...
def sample_random_requests(
    ...
    # 应用重复逻辑  
    if repeat_count > 1:  
        input_requests = repeat_sample_requests(input_requests, repeat_count, repeat_mode)  
        # 截断到目标数量  
        input_requests = input_requests[:num_prompts]  
      
    # 确保最终数量达到要求（如果需要）  
    while len(input_requests) < num_prompts:  
        input_requests.extend(input_requests[:num_prompts - len(input_requests)])  
  
    print(f"#Input tokens: {sum(req.prompt_len for req in input_requests)}")  
    print(f"#Output tokens: {sum(req.output_len for req in input_requests)}")  
...
async def benchmark(
    ...
    if args.output_file:
        ...
        if hasattr(args, 'repeat_count') and args.repeat_count > 1:  
            repeat_suffix = f"_repeat{args.repeat_count}_{args.repeat_mode}"  
            if args.dataset_name.startswith("random"):  
                output_file_name = f"{args.backend}_{now}_{args.num_prompts}_{args.random_input_len}_{args.random_output_len}{repeat_suffix}.jsonl"  
            else:  
                output_file_name = f"{args.backend}_{now}_{args.num_prompts}_sharegpt{repeat_suffix}.jsonl"
...
if __name__ == "__main__":
    ...
    parser.add_argument(  
        ...
        type=int,  
        default=1,  
        help="Number of times to repeat each prompt for performance testing"  
    )  
    parser.add_argument(  
        "--repeat-mode",  
        type=str,  
        choices=["duplicate", "sequential", "interleaved"],  
        default="duplicate",  
        help="Mode for repeating prompts: duplicate (same prompt N times), sequential (repeat dataset N times), interleaved (repeat each prompt before next)"  
    )
